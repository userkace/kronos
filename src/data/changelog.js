...
export const CHANGELOG = [
  {
    ...
    changes: [
      ...
      { type: 'fixed', description: 'The world map in the timezone picker draws properly again. Its old map supplier began requiring an account and quietly served a placeholder image in place of the map, so Kronos has switched to a different one. The water now runs cleanly out to both edges at the default zoom, and the small credit line those maps require sits in the corner.' },
    ],
  },
  {
    version: '0.8.8',
  ...
  },
  {
    ...
    changes: [
      ...
      { type: 'fixed', description: 'The orange break markers between tasks no longer lag behind the list and slide over the tasks on either side of them. Breaks and task cards both animate to their new positions whenever something above them changes height, but breaks were taking four tenths of a second while the cards took three — so a break arrived a beat late and crossed into its neighbours on the way, which read as it ghosting into the task above or below. Everything in the list now moves for exactly the same length of time, so a break holds its place in the gap the whole way.' },
      { type: 'fixed', description: 'Clicking the email field on the invoice page no longer crashes the app.' },
    ],
  },
  {
    version: '0.8.3',
